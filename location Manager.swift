...
import SwiftDate
  ...
  var longitude = 0.0
  var latitude = 0.0
var birthTimeZone = TimeZone(identifier: "America/Los_Angeles")


  func geocoding(location: String,  completion: @escaping () -> ())  {
...
//  let datePicker = datePicker.date
  var currentTimeZone = timeZone
   ...
   var birthTimeMeridian = -timeZone * 15
...
//    105 x 4 = 420 minutes
    ...
    let standardTimeAtBirth = now.myBirthDate()
    let standardMeridian = 0
    let birthLongitude = longitude - Double(standardMeridian)
let lmtInMinutes = birthLongitude * 4
    
    var meridianOffSet = Double(birthTimeMeridian) - birthLongitude
    var egmt = meridianOffSet * 4
    
   var timeZoneAtBirth = (birthTimeMeridian / 15) * 60
    
    
    var  preLMToffset = meridianOffSet * 4
     
    var EGMTIntervals = standardTimeAtBirth + timeZoneAtBirth.hours
    
   var LMToffset =  preLMToffset * 60
    
    var localMeamTimeOfBirth = standardTimeAtBirth + Int(LMToffset).seconds
    
    print("standard meridian = \(standardMeridian)")
    print("birthTimeMeridian = \(birthTimeMeridian)")
    print("meridianOffSet = \(meridianOffSet)")
    print("LMToffset = \(LMToffset)")
    print("emgt = \(egmt)")
    print("emgt = \(now.myBirthDate())")
    print("standard time at birth = \(standardTimeAtBirth)")

    print("localMeamTimeOfBirth = \(localMeamTimeOfBirth)")
    
    print("EGMTIntervals = \(EGMTIntervals)")
    print("timeZoneAtBirth = \(timeZoneAtBirth)")
...
//    var EGMT = now.myBirthDate().timeIntervalSince1970 + timeZoneAtBirth
  
//  let formatted134 = EGMT.formatted // May 26, 2022, 8:16 PM          newFormatter.timeZone =  TimeZone(abbreviation: "UTC")
...
//  Rule 15. Divide the ° by 15. The quotient is hours, the remainder multiplied by 4 is minutes.
    ...
    print("distanceFromGMTInSeconds = \(distanceFromGMTInSeconds)")
    print("standar distanceFromGMTInHoursAndMinutes = \(distanceFromGMTInHoursAndMinutes)")
    print("standar meridian = \(lmtInMinutes)")

    print("EGMT = \(egmt)")
  
   
    print("standardTimeAtBirth ST = \(standardTimeAtBirth)")
    print("distanceFromGMTInSeconds = \(distanceFromGMTInSeconds)")
...
//          If the EGMT Interval of birth is minus, add the calendar interval thus found to the year and month of birth. If the EGMT Interval of birth is plus, subtract the calendar interval thus found from the year, month and day of birth. The L.D. thus found is the calendar starting point from which all major-progressed aspects and positions are calculated.
  ...
  // convert to Integer
  let EGTMInSeconds = timeInterval1
...
//          if longitude <= 0.0  {
    ...
    var EGMTInterval = NoonInterval!.timeIntervalSince1970 - EGMTIntervals.timeIntervalSince1970
   
    
    var forMe = NoonInterval! - EGMTIntervals
...
//            var LimitingDate = limitingDateNumber * 60
    ...
    print("forMe \(forMe)")
    print("EGMTInterval date \(EGMTInterval)")
    print("limiting date \(limitingDate)")
    print("LDinSeconds date \(LDinSeconds)")
...
//             var furureLimiting = LM + preLimitingDate
    ...
    print("standard meridian = \(NoonInterval)")
    print("standard meridian = \(EGMTInterval)")
    print("EGMT = \(egmt)")
...
//
    ...
    func getMyProgressedPlanets (date: Date)  -> [String] {
...
var progressed_sunCoordinate = Coordinate<Planet>(body: .sun, date: date)
 var progressed_sunSign = progressed_sunCoordinate.tropical.formatted
        ...
        var progressed_AscCoordinate = my_ascDegree
        var progressed_AscSign = my_house_ascDegree 
...
var progressed_venusSign = progressed_venusCoordinate.tropical.formatted
 var progressed_marsCoordinates = Coordinate<Planet>(body: .mars, date: date)
var progressed_marsSign = progressed_marsCoordinates.tropical.formatted
var progressed_jupiterCoordinates = Coordinate<Planet>(body: .jupiter, date: date)

var progressed_jupiterSign = progressed_jupiterCoordinates.tropical.formatted
 var progressed_saturnCoordinates = Coordinate<Planet>(body: .saturn, date: date)
var progressed_saturnSign = progressed_saturnCoordinates.tropical.formatted
  var progressed_uranusCoordinates = Coordinate<Planet>(body: .uranus, date: date)
...
var progressed_neptuneSign = progressed_neptuneCoordinate.tropical.formatted
      var progressed_plutoCoordinates = Coordinate<Planet>(body: .pluto, date: date)
      var progressed_plutoSign = progressed_plutoCoordinates.tropical.formatted
      
     

      var myProgPlanets =
        [progressed_sunSign,progressed_moonSign,progressed_AscSign,progressed_mercurySign, progressed_venusSign,progressed_marsSign,progressed_jupiterSign,progressed_saturnSign,progressed_uranusSign,progressed_neptuneSign,progressed_plutoSign]
